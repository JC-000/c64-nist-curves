...
| `lim_lee_comb_p256` | 16384 | REU | `src/points256_comb.s` (`ec_precompute_256`) | All **except** `LIB_SHA384_ONLY` (issue #88) — the onchip full archive still populates/reads REU bank `$02` (only the mul banks `$00`/`$01` disappear under onchip) | Curve-specific (P-256) | h=8 Lim-Lee fixed-base scalar-mul anchor table for `secp256r1` at REU bank `$02` offset `$0000`. 256 entries × 64 B (X, Y only — no Z). Specific to the P-256 generator point and curve parameters; not shareable across curves. Built once at boot; only consumed by the fixed-base scalar_mul path in `lib-p256`. Excluded from `lib-p256-verify` archive per `API.md` §8.3. |
| `lim_lee_comb_p384` | 24576 | REU | `src/points384_comb.s` (`ec_precompute_384`) | All **except** `LIB_SHA384_ONLY` (issue #88) — same REU bank `$02` note as `lim_lee_comb_p256` | Curve-specific (P-384) | h=8 Lim-Lee fixed-base scalar-mul anchor table for `secp384r1` at REU bank `$02` offset `$4000`. 256 entries × 96 B (X, Y only — no Z). Specific to the P-384 generator point and curve parameters; not shareable across curves. Built once at boot; only consumed by the fixed-base scalar_mul path in `lib-p384`. Excluded from `lib-p384-verify` archive per `API.md` §8.3. |
| `sha384_k` | 640 | RODATA | `src/sha384.s` | All, including `LIB_SHA384_ONLY` — the only row that variant emits | Algorithm-specific (SHA-384/512) | FIPS 180-4 §4.2.3 K[80] round constants for the SHA-512 compression family (SHA-384 reuses the same K table; only the IV differs). 80 × 8 B little-endian. Could in principle be shared with a future SHA-512 sibling library, but no second adopter exists today (TLS 1.3 secp384r1 pairs with SHA-384 only). Promotion to §8.x would require a second adopter and an audit-confirmed bit-identical table; not pursued in this release. |

Three build variants of this manifest ship, each enumerating exactly the
tables its archives contain. Export counts are per-object totals, and
each table contributes **six** symbols since contract v0.7.0 — the
prefixed `LIB_NISTCURVES_PRECALC_<name>_{SIZE,REGION,SHARED}` triple plus
the deprecated bare one:
