...
- **v0.11.0** (2026-08-15) — **P-256 comb archives** (issue #117, requested by the c64-https consumer): `make lib-p256-comb` / `make lib-p256-comb-onchip` make the fastest verify configuration (comb-accelerated `u1·G`, 12.4 s @64 MHz consumer-measured) reachable through §6.1 targets instead of member surgery, each with its own measured §6.4 manifest (`LIB_P256_COMB_ONLY`; ZP 17 B, RESIDENT 9216, COLD 1050/870, REU `$07`/`$04` — the comb bank stays claimed in both profiles) and per-curve `lim_lee_comb_*` precalc rows. Also corrects the vic_blank figure to ~6.3% NTSC / ~5.5% PAL (issue #116). PRG byte-identical, ABI stays 2. See [`docs/RELEASE_NOTES_v0.11.0.md`](docs/RELEASE_NOTES_v0.11.0.md).
- **v0.10.2** (2026-08-15) — PATCH, PRG byte-identical, ABI stays 2. Post-wave SPEC conformance (v0.10.0 §6.6/§6.7 → v0.10.3): two footprint equates corrected safe-direction (sha384 RESIDENT 9000→9216, onchip-verify COLD 240→250 — §6.6 deltas per profile×variant in the notes), the sqtab-window guard re-based source-level via `src/sqtab_base.inc` (its old import was a next-MAJOR time bomb), the R2 ZP alias audit as a standing ratchet (fleet scoreboard complete), the §1 version-identity check, and the §6.6 consumer fit-check example. See [`docs/RELEASE_NOTES_v0.10.2.md`](docs/RELEASE_NOTES_v0.10.2.md).
- **v0.10.1** (2026-08-15) — PATCH, zero functional change; **pin this instead of v0.10.0**, which ships content self-reporting as 0.10.1 (`VERSION_PATCH = 1` carried over from v0.9.1; caught by lib-contract #76 ref verification, documented-as-wrong per the tags-are-immutable convention). `VERSION` file aligned; `make check-archives` gains a mechanical §1 identity check (VERSION file vs built equates, all three components) so a partial bump can no longer pass a partial reading. See [`docs/RELEASE_NOTES_v0.10.1.md`](docs/RELEASE_NOTES_v0.10.1.md).
- **v0.10.0** (2026-08-15) — **Namespace-wave release**: closes the fleet's §6.5 rename window as its last adopter (lib-contract #76: polyval v0.6.0 → chacha v0.8.0 → x25519 v0.11.0 → nist). Canonical `nistcurves_zp_*` / `nistcurves_mul_*` names with bare forms export-gated under `-D LIB_NO_BARE_EXPORTS=1` — a gated build exports **zero** unprefixed names, making the c64-https pair collision-free for the #82/#83 families. **`LIB_ABI_VERSION` 1→2** (three colliding unprefixed §8.2 equates removed; prefixed `LIB_NISTCURVES_SHARED_REU_MUL_*` outputs replace them). New `CONTRACT_DEFINES`/`CONTRACT_ZP_DEFINES` build interface, required `lib-app-owned` target, `SHARED_REU_MUL_FETCH` deferral. PRG 37427 → 37480 B (#102 restores 53 bytes the image was short; 1090/1090 oracle checks). Plus the sqtab-window link guard, SPEC §4 cfg declarations, and `make check-docs`. See [`docs/RELEASE_NOTES_v0.10.0.md`](docs/RELEASE_NOTES_v0.10.0.md).
- **v0.9.1** (2026-08-14) — PATCH. Re-copies `src/precalc_table.inc` from c64-lib-contract v0.7.4, which pins the `LIB_PRECALC_TABLE` macro's `_REGION` / `_SHARED` exports `: abs`. Both are byte-valued, so ca65 inferred *zeropage* while a consumer's `.import` defaults to absolute — making SPEC §8.4's own cross-check snippet emit `ld65: Warning: Address size mismatch` on every composed build. Diagnostic noise rather than breakage, but the natural fix a consumer reaches for pins a manifest constant to an address size that is an artifact of its value. `_SIZE` stays deliberately unhinted so the 131072-byte `reu_mul` table still exports `far`. No symbol or value change; PRG byte-identical to v0.9.0. See [`docs/RELEASE_NOTES_v0.9.1.md`](docs/RELEASE_NOTES_v0.9.1.md).
- **v0.9.0** (2026-08-13) — **Manifest honesty release**: of the 45 SPEC §5 manifest values a consumer can read across the nine archives, only two were correct. One manifest object described the whole library and shipped into every archive, so eight of nine advertised resources they do not contain — `COLD_BYTES` was 19% low even for the full archive (outside §5's ±5% band), `REU_BANKS_USED` was wrong in 6 of 9 (the onchip verify/curve archives issue zero REU DMA yet claimed a bank), precalc rows were false in 6 of 9 (a P-256-only archive advertised the 24 KB P-384 comb table), and `ZP_USAGE_BYTES` was overstated up to 2.3×. Per-variant manifests now give each archive figures true to its own contents (issues #88/#90). Also adopts c64-lib-contract v0.5.0→v0.7.1 (`LIB_NISTCURVES_SHARED_CONSUMES`, library-prefixed manifest exports; issue #86) and deletes 384 B of duplicated RFC 6979 vectors that shipped in 7 of 9 archives (issue #91 — the test suite's copies are retained). **Two ABI-surface removals, 17 exported symbols**, under one MINOR bump, signalled by `LIB_ABI_VERSION` `0` → `1` (this library had been the outlier at `0` since v0.3.0; every sibling adopter uses `1`). PRG 37683 → 37427 B; 1090/1090 oracle checks pass. See [`docs/RELEASE_NOTES_v0.9.0.md`](docs/RELEASE_NOTES_v0.9.0.md).
- **v0.8.0** (2026-07-28) — **Archive honesty release**: the SPEC §8.2 `reu_mul` provider (`reu_mul_init` / `reu_mul_tables_init`) now ships in the default-profile archives (issue #81 — previously API.md §3 made `jsr reu_mul_init` mandatory yet no archive contained the symbol), the FP_ONCHIP_MUL manifests stop claiming the §8.2 ownership bit and `reu_mul` precalc row they never use (issue #78, mask `$0007` → `$0005` under the profile), and the reference docs were resynced to the shipped build (issue #77 — the API.md §2 memory map was ~$3000 stale) with turbo/onchip floor + crossover figures scoped to their C64U fw 1.1.0 measurement (issue #83 part 2). §5 `COLD_BYTES` re-baselined 2500 → 1800 net. No ABI change; standalone PRG stays 37683 B. See [`docs/RELEASE_NOTES_v0.8.0.md`](docs/RELEASE_NOTES_v0.8.0.md).
- **v0.7.0** (2026-07-25) — ECDSA verify **public-key validation gate** (issue #66, FIPS 186-5 §3.3): `ecdsa_verify_256/384` now reject non-canonical (`Qx ≥ p` / `Qy ≥ p`) and off-curve public keys with C=1 before any scalar multiplication, closing the last unvalidated-input gap in the packaged verifiers. +512 B PRG (37171 → 37683 B), no new RAM, both comb and `ECDSA_NO_COMB` variants covered; 8 new negative-Q oracle tests per suite. Also widens the documented `fp_mod_mul_n[_384]` precondition to at-least-one-operand-reduced (issue #65). See [`docs/RELEASE_NOTES_v0.7.0.md`](docs/RELEASE_NOTES_v0.7.0.md).
- **v0.6.0** (2026-07-20) — Turbo-profile row generator upgraded to an inline quarter-square (issue #71): `ecdsa_verify_256` @64 MHz drops to **11.8 s (2.16× vs the DMA-table default)**, crossovers ~22 MHz (P-256) / ~33 MHz (P-384). Zero-REU verify configuration runtime-proven (35/35 oracle suite in an REU-less VICE via `make onchip-nocomb-prg` + `C64_NO_REU=1`). Fixes the silently-broken `ct_mul_brute_check.py` shim (now PASS 65536/65536). Default profile byte-identical. See [`docs/RELEASE_NOTES_v0.6.0.md`](docs/RELEASE_NOTES_v0.6.0.md).
