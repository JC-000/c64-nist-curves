...
reu_fetch_mul_row:
        ...
        sta reu_command
        jsr nistcurves_reu_dma_wait ; SPEC v0.13.0 §8.2 (a)+(b): caller's next REU access unknown
        rts
.endif ; SHARED_REU_MUL_FETCH

; nistcurves_reu_dma_wait moved to src/data_reu_wait.s by issue #155.
; It used to live here, and because fp256.o / fp384.o import it, EVERY link
; that called a field operation pulled this member in -- and this member also
; exports the displaceable `sqtab_lo` / `sqtab_hi`, so a sibling library
; exporting the same bare §8.1 names collided with no consumer definition
; involved anywhere (SPEC 1.2.2 §6.1, the library-versus-library direction).
; Demonstrated: a consumer calling only `fp_mul`, linked beside a sibling that
; exports bare sqtab_lo/sqtab_hi, failed with
;   ld65: Error: Duplicate external identifier: 'sqtab_hi'
; while the same consumer calling only `sha384_init` linked clean.
; The routine now lives beside the state it uses, in a TU with no displaceable
; exports, so resolving it can never drag sqtab or the §8.3 body along.
