...
@halfu:
        lda fp384_inv_u
        beq :+
        jmp @halfu_low_nz                ; low byte != 0 -> per-bit path
:
        ; --- u low byte == 0 fast path (P-384, 48-byte): byte-shift u down,
        ; then halve x1 by 2^8 mod m via 8 single-bit halvings inline.
        ldx #0
@halfu_bshift:
        lda fp384_inv_u+1,x
        sta fp384_inv_u,x
...
.endrepeat
        dec fp_loop                      ; DEC preserves carry; outer loop, unused
        beq :+
        jmp @halfu_x1_8loop
:
        jmp @halfu
...
@halfv:
        lda fp384_inv_v
        beq :+
        jmp @halfv_low_nz                ; low byte != 0 -> per-bit path
:
        ; v low-byte zero fast path: byte-shift v, then 8 single-bit halvings of x2.
        ldx #0
@halfv_bshift:
        lda fp384_inv_v+1,x
        sta fp384_inv_v,x
