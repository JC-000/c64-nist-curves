...
**Fixed in v0.14.0** by issue #154 (`src/zp_aliases.s`) — it was deferred out of
...
- **1.2.0 §6.1 member isolation** (as corrected by **1.2.1**) — a symbol a consumer may displace (gated
  ...
  **1.2.2** ruled the one §6.1 case v0.13.0 shipped open (our issue #154,
  upstream contract#188): `zp_config.o` exported the four bare `zp_*` aliases
  beside sixteen importable slots. §2's dedicated-file requirement governs
  *claimed slots* and a bare alias is not one, so the aliases moved to
  `src/zp_aliases.s` — **archived**, which is what makes it a relocation
  rather than a removed export owing a §6.5 window. **Do not move them to a
  never-archived TU.**
...
- **FP_ONCHIP_MUL turbo profile** (issue #69, v0.5.0): `-D FP_ONCHIP_MUL`
  ...
  issue #83.
  Ships as five `make lib-*-onchip` archives (incl. `lib-p256-comb-onchip`,
  issue #117) with a profile-aware
